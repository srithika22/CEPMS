...
import './smartEventManagement.css';
...
const FacultyDashboard = () => {
  ...
  const fetchSessions = async () => {
    try {
      const response = await axios.get(`${API_BASE}/sessions`, getAuthHeaders());
  ...
  const fetchAttendanceData = async () => {
    try {
      const response = await axios.get(`${API_BASE}/attendance/faculty`, getAuthHeaders());
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await axios.get(`${API_BASE}/analytics/faculty`, getAuthHeaders());
  ...
  const calculateStats = () => {
    ...
    const pendingApproval = myEvents.filter(e => e.status === 'pending').length;
    const approvedEvents = myEvents.filter(e => e.status === 'approved').length;
    const totalParticipants = myEvents.reduce((total, event) => 
          ...
          <div key={event._id} className="event-card">
            ...
            <div className="event-details">
              ...
              <div className="detail-row">
                ...
                <span>{event.coordinatorName || 'Unknown'}</span>
              </div>
              <div className="detail-row">
                <span className="label">🎯 Sessions:</span>
                <span>{sessions.filter(s => s.eventId === event._id).length} sessions</span>
              </div>
            </div>
                ...
                <div className="attendance-details">
                  <div className="attendance-stats">
                    ...
                    <div className="stat">
                      ...
                      <span className="stat-value">{session.venue}</span>
                    </div>
                  </div>
